# GT2 — LLM Fixed-N Split + Multi-Vector ERC
...
câu hỏi
  ...
  → [embed(q_gốc), embed(sub_1), ..., embed(sub_N)]  ← phân phối X (N+1 vectors)
...
**Điểm khác biệt so với GT3_Auto:** Số sub-queries **cố định** theo `--num-queries` (LLM phải tuân thủ đúng số đó, có `while` loop padding nếu thiếu). GT3_Auto để LLM tự quyết. GT2 giúp đánh giá xem N cụ thể nào cho kết quả tốt nhất.

**Điểm khác biệt so với GT1_1Vector:** GT1 dùng phân phối X gồm 1 vector (E_XX=0), GT2 dùng N+1 vectors (E_XX>0, phân phối thực).

---

## Bước 1 — Sinh file kết quả

Chạy từ thư mục gốc (chứa `GT2_Manual_N/` và `Folder_All/`):

```bash
# N=2 (mặc định)
python -m GT2_Manual_N.create_eval --num-queries 2

# N=1 — đánh giá single sub-query (vẫn có câu gốc → tổng 2 vectors)
python -m GT2_Manual_N.create_eval --num-queries 1 --max 50

# N=3 (4 vectors tổng)
python -m GT2_Manual_N.create_eval --num-queries 3

# N=4 (5 vectors tổng)
python -m GT2_Manual_N.create_eval --num-queries 4

# Tùy chỉnh đầy đủ
python -m GT2_Manual_N.create_eval \
    --num-queries 2 \
    --input  Folder_All/scoring/"file 1000 cau hoi.xlsx" \
    --output GT2_Manual_N/GT2_N2_eval_results.xlsx \
    --llm-provider openai \
...
| `--output` | `GT2_N{N}_eval_results.xlsx` | File xuất — tự động đặt tên theo N nếu không chỉ định |
...
**Output:** `GT2_Manual_N/GT2_N{N}_eval_results.xlsx` với các cột:
...
cd Folder_All
python scoring/main.py --file ../GT2_Manual_N/GT2_N1_eval_results.xlsx
python scoring/main.py --file ../GT2_Manual_N/GT2_N2_eval_results.xlsx
python scoring/main.py --file ../GT2_Manual_N/GT2_N3_eval_results.xlsx
python scoring/main.py --file ../GT2_Manual_N/GT2_N4_eval_results.xlsx
```
...
# Chạy cả 4 cấu hình
for N in 1 2 3 4; do
    python -m GT2_Manual_N.create_eval --num-queries $N --max 100
done

# Chấm điểm tất cả
cd Folder_All
for N in 1 2 3 4; do
    python scoring/main.py --file ../GT2_Manual_N/GT2_N${N}_eval_results.xlsx
done
```
