...
Cách dùng:
    python -m GT2_Manual_N.create_eval --num-queries 2 --max 50
    python GT2_Manual_N/create_eval.py --num-queries 3 --output GT3_N3.xlsx
    python GT2_Manual_N/create_eval.py --num-queries 2 --start 991   # tiếp tục từ câu 991
"""

from __future__ import annotations

# Bootstrap: hỗ trợ cả `python -m` và `python GT2_Manual_N/create_eval.py`
if __name__ == "__main__" and __package__ is None:
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    __package__ = "GT2_Manual_N"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GT2: LLM Fixed-N → Multi-Vector ERC")
    parser.add_argument("--input", default=str(PROJECT_ROOT / "scoring" / "file 1000 cau hoi.xlsx"))
    parser.add_argument("--output", default=None, help="Mặc định: GT2_N{N}_eval_results.xlsx")
    ...
    output_path = args.output or str(HERE / f"GT2_N{args.num_queries}_eval_results.xlsx")
    ...
    run_eval(
        ...
        llm=llm,
        algorithm=f"GT2_llm_fixed_{args.num_queries}_erc",
