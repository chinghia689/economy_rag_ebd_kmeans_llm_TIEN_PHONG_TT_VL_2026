# GT1 — LLM Auto Split + Multi-Vector ERC

## Thuật toán

```
câu hỏi
  → LLM tự quyết 1–4 sub-queries
...
**Điểm khác biệt so với GT2/GT3:** LLM tự quyết số lượng sub-queries (không cố định), tối đa `--max-parts`.

---

## Bước 1 — Sinh file kết quả

Chạy từ thư mục gốc (chứa `GT1_LLM_Auto/` và `Folder_All/`):

```bash
# Cơ bản — chạy toàn bộ 1000 câu hỏi
python -m GT1_LLM_Auto.create_eval

# Chạy thử 50 câu
python -m GT1_LLM_Auto.create_eval --max 50

# Tùy chỉnh đầy đủ
python -m GT1_LLM_Auto.create_eval \
    --input  Folder_All/scoring/"file 1000 cau hoi.xlsx" \
    --output GT1_LLM_Auto/GT1_eval_results.xlsx \
    --max-parts 4 \
    --llm-provider openai \
...
| `--output` | `GT1_LLM_Auto/GT1_eval_results.xlsx` | File Excel kết quả |
...
**Output:** `GT1_LLM_Auto/GT1_eval_results.xlsx` với các cột:
...
```bash
cd Folder_All
python scoring/main.py --file ../GT1_LLM_Auto/GT1_eval_results.xlsx
```
