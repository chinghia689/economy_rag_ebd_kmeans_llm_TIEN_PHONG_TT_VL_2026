...
Chạy từ thư mục gốc (chứa `Gt2_mutiquery/` và `Folder_All/`):
...
python -m Gt2_mutiquery.create_eval \
    ...
    --output Gt2_mutiquery/Gt2_mutiquery_eval_results.xlsx \
    --max-parts 4 \
    --llm-provider openai \
...
| `--output` | `Gt2_mutiquery/Gt2_mutiquery_eval_results.xlsx` | File Excel kết quả |
...
**Output:** `Gt2_mutiquery/Gt2_mutiquery_eval_results.xlsx` với các cột:
...
```bash
cd Folder_All
python scoring/main.py --file ../Gt2_mutiquery/Gt2_mutiquery_eval_results.xlsx
```
...
Gt2_mutiquery **không phải** thuật toán nghiên cứu chính của luận văn — đây là module demo dùng cho frontend production (`Folder_All/chatbot/`). Hai thuật toán nghiên cứu chính là **GT1_1Vector** (phân phối suy biến) và **GT2_Manual_N** (auto-split với giới hạn do người dùng chọn).
